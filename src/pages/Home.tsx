...
function useRestaurants(category: CategoryFilter, page: number) {
  return useQuery({
    queryKey: ["restaurants", category, page],
    queryFn: async () => {
      let query = supabase
        .from("restaurants")
        .select("*", { count: "exact" })
...
export default function Home() {
  ...
  const { data, isLoading, isError } = useRestaurants(category, page);
    ...
    <div dir="rtl" className="min-h-screen bg-[#F8F8F8]">
      ...
      <section className="bg-gradient-to-b from-orange-50 to-white py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto text-center">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-[#1A1A2E] mb-4 leading-tight">
            اكتشف أحسن المطاعم في مدينتك
          </h1>
          <p className="text-gray-500 text-base sm:text-lg mb-8">
            تصفح قوائم الطعام، اقرأ التقييمات، وجد أقرب مطعم
          </p>
          ...
          >
            <input
              ...
              placeholder="ابحث عن مطعم في مدينتك..."
              className="w-full pl-12 pr-4 py-4 rounded-full border border-gray-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-[#FF6B35] focus:border-transparent text-base bg-white"
            />
            <button
              type="submit"
              className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#FF6B35] transition-colors"
              aria-label="بحث"
            >
              <svg
                className="w-6 h-6"
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-500 text-lg">لا توجد مطاعم حالياً</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
